...
for (const code of codes) {
  ...
  if (!fs.existsSync(file)) { errors.push(`Eksik modal rotas?: ${code}`); continue; }
  const html = fs.readFileSync(file, 'utf8');
  if (!/<meta\s+name=["']robots["']\s+content=["']noindex,follow["']/i.test(html)) errors.push(`Modal rotas? noindex de?il: ${code}`);
...
for (const entry of fs.readdirSync(detailRoot, { withFileTypes: true })) {
  ...
  if (!sku) continue;
  detailCodes.add(sku);
  if (!codeSet.has(sku)) errors.push(`Detay sayfas? katalogda yok: ${sku}`);
}

const sitemap = fs.readFileSync(path.join(root, 'sitemap.xml'), 'utf8');
const modalInSitemap = [...sitemap.matchAll(/<loc>([^<]*\/urunlerimiz\/kt-[^<]+)<\/loc>/gi)].map((m) => m[1]);
if (modalInSitemap.length) errors.push(`Sitemap modal rotas? i?eriyor: ${modalInSitemap.slice(0, 3).join(', ')}`);
